...
public class MainWindowController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        miARHI.setOnAction(new EventHandler() {
            ...
            public void handle(Event event) {
                ...
                System.out.println(Main.lmbIdent.toString());
            }
        });
    ...
    private void submitButton(ActionEvent event) {
        ...
        ResultSet rs = DatabaseConnector.getInstance().request("SELECT userId, userName, password FROM user WHERE userName=\"" + username + "\" AND password=\"" + password + "\"");
        try {
            if(rs.next()) {
            ...
            if(Main.langIdent == LanguageIdentifier.FRENCH) {
                Main.newError("Erreur", "Euh oh! Quelque chose d'inattendu vient de se passer. Essayez de vous reconnecter dans une minute.");
            } else if(Main.langIdent == LanguageIdentifier.SPANISH) {
                Main.newError("Error", "¡Uh oh! Algo inesperado acaba de suceder. Intente iniciar sesión nuevamente en un minuto.");
            } else {
                Main.newError("Error", "Uh oh! Something unexpected just happened. Try logging in again in a minute...");
            }
        }
    }
    ...
    private void usernameNotFound() {
        if(Main.langIdent == LanguageIdentifier.FRENCH) {
            Main.newError("Nom d'utilisateur ou mot de passe invalide","Nom d'utilisateur ou mot de passe invalide, veuillez réessayer.");
        } else if(Main.langIdent == LanguageIdentifier.SPANISH) {
            Main.newError("Usuario o contraseña invalido", "Nombre de usuario o contraseña no válidos, intente nuevamente.");
        } else {
    ...
    private String xor(String toXOR) {
        String toReturn = "";
        //This (xord) is in plain text, within the application and can be decrypted very, Very, VERY easily.. Please update uCertify to allocate space for passwords to be hashed, kthx.
        //   In case you don't understand, please change the datatype from varchar to binary for the user's password column in the MySQL database.
